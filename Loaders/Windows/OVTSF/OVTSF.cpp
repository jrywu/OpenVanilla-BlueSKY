...
/* static */
HRESULT COVTSF::CreateInstance(_In_ IUnknown *pUnkOuter, REFIID riid, _Outptr_ void **ppvObj)
{
    COVTSF* pOVTSF = nullptr;
    HRESULT hr = S_OK;
    ...
    *ppvObj = nullptr;
    ...
    pOVTSF = new (std::nothrow) COVTSF();
    if (pOVTSF == nullptr)
    {
        return E_OUTOFMEMORY;
    }

    hr = pOVTSF->QueryInterface(riid, ppvObj);
...
STDAPI COVTSF::QueryInterface(REFIID riid, _Outptr_ void **ppvObj)
{
    if (ppvObj == nullptr)
    {
        return E_INVALIDARG;
...
STDAPI COVTSF::ActivateEx(ITfThreadMgr *pThreadMgr, TfClientId tfClientId, DWORD dwFlags)
{
...
//
//----------------------------------------------------------------------------
HRESULT COVTSF::GetType(__RPC__out GUID *pguid)
{
    HRESULT hr = E_INVALIDARG;
    if (pguid)
    {
        *pguid = Global::OVTSFCLSID;
        hr = S_OK;
    }
    return hr;
}
...
//
//----------------------------------------------------------------------------
HRESULT COVTSF::GetDescription(__RPC__deref_out_opt BSTR *pbstrDesc)
{
    HRESULT hr = E_INVALIDARG;
...
//
//----------------------------------------------------------------------------
HRESULT COVTSF::GetFunction(__RPC__in REFGUID rguid, __RPC__in REFIID riid, __RPC__deref_out_opt IUnknown **ppunk)
{
...
//
//----------------------------------------------------------------------------
HRESULT COVTSF::GetDisplayName(_Out_ BSTR *pbstrDisplayName)
{
    HRESULT hr = E_INVALIDARG;
...
//----------------------------------------------------------------------------
HRESULT COVTSF::GetLayout(_Out_ TKBLayoutType *ptkblayoutType, _Out_ WORD *pwPreferredLayoutId)
{
    HRESULT hr = E_INVALIDARG;
