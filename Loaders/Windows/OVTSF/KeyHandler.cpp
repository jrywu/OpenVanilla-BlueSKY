...
{
	murmur("KeyHandler:CKeyHandlerEditSession::DoEditSession(),Processing key = %d", _wParam);
...
{
	murmur("KeyHandler: CTextService::_HandleCharacterKey()");
...
{
	murmur("KeyHandler: CTextService::_HandleReturnKey()");
...
{
	murmur("KeyHandler: CTextService::_HandleSpaceKey()");
  //
  // set the display attribute to the composition range.
  //
...
{
	murmur("KeyHandler: CTextService::_HandleArrowKey()");
    ITfRange *pRangeComposition;
    LONG cch;
    BOOL fEqual;
    TF_SELECTION tfSelection;
...
{
	murmur("KeyHandler: CTextService::_InvokeKeyHandler()");
