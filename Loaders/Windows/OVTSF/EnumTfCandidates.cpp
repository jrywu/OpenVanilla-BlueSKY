...
HRESULT CEnumTfCandidates::CreateInstance(_Out_ CEnumTfCandidates **ppobj, _In_ const COVTSFArray<ITfCandidateString*> &rgelm, UINT currentNum)
{
    if (ppobj == nullptr)
    {
        return E_INVALIDARG;
...
HRESULT CEnumTfCandidates::CreateInstance(REFIID riid, _Out_ void **ppvObj, _In_ const COVTSFArray<ITfCandidateString*> &rgelm, UINT currentNum)
{
    if (ppvObj == nullptr)
    {
        return E_POINTER;
...
CEnumTfCandidates::CEnumTfCandidates(_In_ const COVTSFArray<ITfCandidateString*> &rgelm, UINT currentNum)
{
    _refCount = 0;
    _rgelm = rgelm;
