...
class COVTSFArray
{
    typedef typename std::vector<T> COVTSFInnerArray;
    typedef typename std::vector<T>::iterator COVTSFInnerIter;

public:
    COVTSFArray(): _innerVect()
    {
    }

    explicit COVTSFArray(size_t count): _innerVect(count)
    {
    }
    ...
    void RemoveAt(size_t index)
    {
        ...
        COVTSFInnerIter iter = _innerVect.begin();
...
private:
    COVTSFArray<DWORD> _CandidateListIndexRange;
};
