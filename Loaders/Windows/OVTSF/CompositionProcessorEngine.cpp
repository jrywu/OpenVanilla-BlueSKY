...
BOOL COVTSF::_AddTextProcessorEngine()
{
    LANGID langid = 0;
...
BOOL CCompositionProcessorEngine::SetupLanguageProfile(LANGID langid, REFGUID guidLanguageProfile, _In_ ITfThreadMgr *pThreadMgr, TfClientId tfClientId, BOOL isSecureMode, BOOL isComLessMode)
{
	...
	InitializeOVTSFCompartment(pThreadMgr, tfClientId);
...
void CCompositionProcessorEngine::GetReadingStrings(_Inout_ COVTSFArray<CStringRange> *pReadingStrings, _Out_ BOOL *pIsWildcardIncluded)
{
...
void CCompositionProcessorEngine::GetCandidateList(_Inout_ COVTSFArray<CCandidateListItem> *pCandidateList, BOOL isIncrementalWordSearch, BOOL isWildcardSearch)
{
...
void CCompositionProcessorEngine::GetCandidateStringInConverted(CStringRange &searchString, _In_ COVTSFArray<CCandidateListItem> *pCandidateList)
{
    if (!IsDictionaryAvailable())
    {
        return;
...
void CCompositionProcessorEngine::SetKeystrokeTable(_Inout_ COVTSFArray<_KEYSTROKE> *pKeystroke)
{
...
void CCompositionProcessorEngine::SetupPreserved(_In_ ITfThreadMgr *pThreadMgr, TfClientId tfClientId)
{
    ...
    SetPreservedKey(Global::OVTSFGuidImeModePreserveKey, preservedKeyImeMode, Global::ImeModeDescription, &_PreservedKey_IMEMode);
    ...
    SetPreservedKey(Global::OVTSFGuidDoubleSingleBytePreserveKey, preservedKeyDoubleSingleByte, Global::DoubleSingleByteDescription, &_PreservedKey_DoubleSingleByte);
    ...
    SetPreservedKey(Global::OVTSFGuidPunctuationPreserveKey, preservedKeyPunctuation, Global::PunctuationDescription, &_PreservedKey_Punctuation);
...
BOOL CCompositionProcessorEngine::CheckShiftKeyOnly(_In_ COVTSFArray<TF_PRESERVEDKEY> *pTSFPreservedKeyTable)
{
...
void CCompositionProcessorEngine::OnPreservedKey(REFGUID rguid, _Out_ BOOL *pIsEaten, _In_ ITfThreadMgr *pThreadMgr, TfClientId tfClientId)
{
    ...
    else if (IsEqualGUID(rguid, _PreservedKey_DoubleSingleByte.Guid))
    {
        ...
        BOOL isDouble = FALSE;
        CCompartment CompartmentDoubleSingleByte(pThreadMgr, tfClientId, Global::OVTSFGuidCompartmentDoubleSingleByte);
    ...
    else if (IsEqualGUID(rguid, _PreservedKey_Punctuation.Guid))
    {
        ...
        CCompartment CompartmentPunctuation(pThreadMgr, tfClientId, Global::OVTSFGuidCompartmentPunctuation);
...
void CCompositionProcessorEngine::SetupLanguageBar(_In_ ITfThreadMgr *pThreadMgr, TfClientId tfClientId, BOOL isSecureMode)
{
    ...
    CreateLanguageBarButton(dwEnable, Global::OVTSFGuidLangBarDoubleSingleByte, Global::LangbarDoubleSingleByteDescription, Global::DoubleSingleByteDescription, Global::DoubleSingleByteOnIcoIndex, Global::DoubleSingleByteOffIcoIndex, &_pLanguageBar_DoubleSingleByte, isSecureMode);
    CreateLanguageBarButton(dwEnable, Global::OVTSFGuidLangBarPunctuation, Global::LangbarPunctuationDescription, Global::PunctuationDescription, Global::PunctuationOnIcoIndex, Global::PunctuationOffIcoIndex, &_pLanguageBar_Punctuation, isSecureMode);

    InitLanguageBar(_pLanguageBar_IMEMode, pThreadMgr, tfClientId, GUID_COMPARTMENT_KEYBOARD_OPENCLOSE);
    InitLanguageBar(_pLanguageBar_DoubleSingleByte, pThreadMgr, tfClientId, Global::OVTSFGuidCompartmentDoubleSingleByte);
    InitLanguageBar(_pLanguageBar_Punctuation, pThreadMgr, tfClientId, Global::OVTSFGuidCompartmentPunctuation);
    ...
    if (_pCompartmentDoubleSingleByteEventSink)
    {
        _pCompartmentDoubleSingleByteEventSink->_Advise(pThreadMgr, Global::OVTSFGuidCompartmentDoubleSingleByte);
    }
    if (_pCompartmentPunctuationEventSink)
    {
        _pCompartmentPunctuationEventSink->_Advise(pThreadMgr, Global::OVTSFGuidCompartmentPunctuation);
    }
...
void CCompositionProcessorEngine::InitializeOVTSFCompartment(_In_ ITfThreadMgr *pThreadMgr, TfClientId tfClientId)
{
	// set initial mode
    CCompartment CompartmentKeyboardOpen(pThreadMgr, tfClientId, GUID_COMPARTMENT_KEYBOARD_OPENCLOSE);
...
HRESULT CCompositionProcessorEngine::CompartmentCallback(_In_ void *pv, REFGUID guidCompartment)
{
    ...
    if (IsEqualGUID(guidCompartment, Global::OVTSFGuidCompartmentDoubleSingleByte) ||
        IsEqualGUID(guidCompartment, Global::OVTSFGuidCompartmentPunctuation))
    {
...
void CCompositionProcessorEngine::ConversionModeCompartmentUpdated(_In_ ITfThreadMgr *pThreadMgr)
{
    ...
    BOOL isDouble = FALSE;
    CCompartment CompartmentDoubleSingleByte(pThreadMgr, _tfClientId, Global::OVTSFGuidCompartmentDoubleSingleByte);
    ...
    BOOL isPunctuation = FALSE;
    CCompartment CompartmentPunctuation(pThreadMgr, _tfClientId, Global::OVTSFGuidCompartmentPunctuation);
...
//+---------------------------------------------------------------------------
//
// COVTSF::CreateInstance 
//
//----------------------------------------------------------------------------

HRESULT COVTSF::CreateInstance(REFCLSID rclsid, REFIID riid, _Outptr_result_maybenull_ LPVOID* ppv, _Out_opt_ HINSTANCE* phInst, BOOL isComLessMode)
{
    HRESULT hr = S_OK;
    if (phInst == nullptr)
    {
    ...
    *phInst = nullptr;
    ...
    {
        hr = COVTSF::ComLessCreateInstance(rclsid, riid, ppv, phInst);
    }
...
//+---------------------------------------------------------------------------
//
// COVTSF::ComLessCreateInstance
//
//----------------------------------------------------------------------------

HRESULT COVTSF::ComLessCreateInstance(REFGUID rclsid, REFIID riid, _Outptr_result_maybenull_ void **ppv, _Out_opt_ HINSTANCE *phInst)
{
    HRESULT hr = S_OK;
    HINSTANCE OVTSFDllHandle = nullptr;
    ...
    *ppv = nullptr;
    ...
    {
        *phInst = nullptr;
        hr = COVTSF::GetComModuleName(rclsid, wchPath, ARRAYSIZE(wchPath));
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                OVTSFDllHandle = LoadLibraryEx(szExpandedPath, NULL, 0);
                hr = OVTSFDllHandle ? S_OK : E_FAIL;
                if (SUCCEEDED(hr))
                {
                    *phInst = OVTSFDllHandle;
                    FARPROC pfn = GetProcAddress(OVTSFDllHandle, "DllGetClassObject");
                    hr = pfn ? S_OK : E_FAIL;
                    if (SUCCEEDED(hr))
                    {
...
//+---------------------------------------------------------------------------
//
// COVTSF::GetComModuleName
//
//----------------------------------------------------------------------------

HRESULT COVTSF::GetComModuleName(REFGUID rclsid, _Out_writes_(cchPath)WCHAR* wchPath, DWORD cchPath)
{
    HRESULT hr = S_OK;
...
BOOL CCompositionProcessorEngine::IsVirtualKeyKeystrokeCandidate(UINT uCode, _In_ _KEYSTROKE_STATE *pKeyState, CANDIDATE_MODE candidateMode, _Out_ BOOL *pfRetCode, _In_ COVTSFArray<_KEYSTROKE> *pKeystrokeMetric)
{
