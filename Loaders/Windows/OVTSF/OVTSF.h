...
LRESULT CALLBACK COVTSF_WindowProc(HWND wndHandle, UINT uMsg, WPARAM wParam, LPARAM lParam);

class COVTSF : public ITfTextInputProcessorEx,
...
public:
    ...
    // comless helpers
    static HRESULT COVTSF::CreateInstance(REFCLSID rclsid, REFIID riid, _Outptr_result_maybenull_ LPVOID* ppv, _Out_opt_ HINSTANCE* phInst, BOOL isComLessMode);
    static HRESULT COVTSF::ComLessCreateInstance(REFGUID rclsid, REFIID riid, _Outptr_result_maybenull_ void **ppv, _Out_opt_ HINSTANCE *phInst);
    static HRESULT COVTSF::GetComModuleName(REFGUID rclsid, _Out_writes_(cchPath)WCHAR* wchPath, DWORD cchPath);
...
private:
    ...
    BOOL VerifyOVTSFCLSID(_In_ REFCLSID clsid);

    friend LRESULT CALLBACK COVTSF_WindowProc(HWND wndHandle, UINT uMsg, WPARAM wParam, LPARAM lParam);
