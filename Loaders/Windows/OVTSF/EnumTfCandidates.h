...
protected:
    ...
    CEnumTfCandidates(_In_ const COVTSFArray<ITfCandidateString*> &rgelm, UINT currentNum);

    virtual ~CEnumTfCandidates(void);

public:
    // create instance
    static HRESULT CreateInstance(_Out_ CEnumTfCandidates **ppobj, _In_ const COVTSFArray<ITfCandidateString*> &rgelm, UINT currentNum = 0);
    static HRESULT CreateInstance(REFIID riid, _Out_ void **ppvObj, _In_ const COVTSFArray<ITfCandidateString*> &rgelm, UINT currentNum = 0);
...
protected:
    LONG _refCount;
    COVTSFArray<ITfCandidateString*> _rgelm;
